...
const constructors = JSON.parse(fs.readFileSync('./constructors.json', 'utf8'));

db.serialize(() => {
    const stmt = db.prepare('INSERT INTO constructors (position, teamName, seasonPoints, seasonWins) VALUES (?, ?, ?, ?)');
        
    constructors.forEach(constructor => {
        stmt.run(constructor.position, constructor.teamName, constructor.seasonPoints, constructor.seasonWins);
    });
